...
export const GetInventory_Response = Schema.transformOrFail(Schema.instanceOf(Response), Schema.Record({
  key: Schema.String,
  value: Schema.Number.pipe(Schema.int()).pipe(Schema.mutable)
}).pipe(Schema.mutable), {
  strict: true,
  decode: (from, ctx) => Effect.promise(async () => {
    try {
