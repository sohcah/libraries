...
import { generatePathExpression } from "../src/js/generatePathExpression.ts";

test.for([
  ["/depts/{year}/{week}", {}, null, "`/depts/{year}/{week}`"],
  ["/depts/{year}/{week}", { year: t.stringLiteral("year") }, null, '`/depts/${"year"}/{week}`'],
  [
    "/depts/y{year}/{week}",
    {
    ...
    t.newExpression(t.identifier("URLSearchParams"), [t.stringLiteral("search=text")]),
    '`/depts/y${from.year}/${from.week}?${new URLSearchParams("search=text")}`',
  ],
] as const)("generatePathExpression", ([path, pathParameters, queryParameter, expected]) => {
